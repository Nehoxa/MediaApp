  ...
  <AppLayout :statusCode="season.statusCode" :statusMessage="season.statusMessage" >
    <div class="w-full h-full pb-6 text-white">
      ...
      <Link :href="route('serie.showEpisode', [episode.show_id, episode.season_number, episode.episode_number])"
        v-for="episode in season.episodes" :key="episode.id" class="flex-none justify-center xl:flex xl:justify-start m-16">
        <img class="h-115 rounded-lg m-6 xl:mx-0" :src="'https://image.tmdb.org/t/p/original' + episode.still_path" v-if="episode.still_path"/>
