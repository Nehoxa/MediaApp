  ...
  <div class="py-3 w-screen sm:w-129 3xl:w-130 sm:pb-20 sm:mx-4">
    <div @click="showMovie(movie.id)"
      ...
      <div class="h-48 overflow-visible">
        <img class="rounded-3xl shadow-lg w-36 sm:w-auto" :src="'https://image.tmdb.org/t/p/w200' + movie.poster_path" alt="">
      </div>
      <div class="flex flex-col w-1/2 space-y-4">
        <div class="flex justify-between items-start">
          <h2 class="text-2xl font-bold text-white">{{ movie.title }}</h2>
          <div class="bg-yellow-400 font-bold rounded-xl p-2">{{ movie.vote_average ? parseFloat(movie.vote_average.toFixed(1)) : '' }}</div>
        </div>
        <div>
